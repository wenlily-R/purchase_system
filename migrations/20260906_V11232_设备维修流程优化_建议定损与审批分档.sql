...
-- ① 审批分档: 原三级全段(0-100万) → 高金额档 10万+(三级: 部门主管→分管领导→分管领导); 新增 普通档 0-<10万(部门主管1级审批)
...
SELECT 'repair_plan',1,role,username,0,99999,'部门主管-普通维修-1级'
FROM users WHERE name='穆娇' AND is_active=1
AND NOT EXISTS(SELECT 1 FROM approval_flow_config WHERE biz_type='repair_plan' AND label LIKE '%普通维修%')
LIMIT 1;
