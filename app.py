...
def init_db():
    ...
    for _iqc, _iqd in [('access_code', "TEXT DEFAULT ''"), ('auth_token', "TEXT DEFAULT ''"),
                       ('sms_code', "TEXT DEFAULT ''"), ('sms_exp', "TEXT DEFAULT ''"), ('sms_sent_at', "TEXT DEFAULT ''")]:
        if _iqc not in _iqs:
    ...
    # V11.221 设备维修2.0: 大小件验收/验收场景/三方签字/加急线下纸质件归档/使用周期(幂等补列)
    _rp2 = [r[1] for r in conn.execute("PRAGMA table_info(repair_plans)").fetchall()]
    _adds2 = [
        ('item_class', "TEXT DEFAULT ''"),        # 维修物件分类 小件(胶管/阀件)/大件(大型电机/减速机)
        ('accept_scene', "TEXT DEFAULT ''"),      # 验收场景 服务商出厂验收/回厂安装后验收
        ('accept_signs', "TEXT DEFAULT ''"),      # JSON 大件三方签字 [{role,name,opinion,time}]
        ('offline_papers', "TEXT DEFAULT ''"),    # JSON 加急线下签字纸质件归档 [{name,path,by,opinion,time}]
        ('prev_gap_days', "INTEGER DEFAULT 0"),   # 距上次维修完成天数(设备使用周期, 服务商质量评估)
    ]
...
def _inq_sms_cfg():
    """短信通道配置(sys_config): sms_enabled/access_key_id/access_key_secret/sign_name/template_code(阿里云短信)"""
    _c = db()
    _cfg = {}
    for _k in ('sms_enabled', 'sms_access_key_id', 'sms_access_key_secret', 'sms_sign_name', 'sms_template_code'):
        _r = _c.execute("SELECT value FROM sys_config WHERE key=?", (_k,)).fetchone()
        _cfg[_k] = (_r[0] if _r else '') or ''
    _c.close()
    return _cfg


def _inq_sms_send(phone, code):
    """阿里云短信 SendSms(V2017-05-25)。返回 (ok, message)。未开通/未配置 → (False, 原因)"""
    import hmac
    import urllib.parse as _up
    cfg = _inq_sms_cfg()
    if cfg['sms_enabled'] not in ('1', 'true', 'True'):
        return False, '短信通道未开通'
    ak, sk, sn, tc = cfg['sms_access_key_id'], cfg['sms_access_key_secret'], cfg['sms_sign_name'], cfg['sms_template_code']
    if not (ak and sk and sn and tc):
        return False, '短信通道未配置完整(缺AccessKey/签名/模板)'
    try:
        params = {'AccessKeyId': ak, 'Action': 'SendSms', 'Format': 'JSON', 'PhoneNumbers': phone,
                  'RegionId': 'cn-hangzhou', 'SignName': sn, 'SignatureMethod': 'HMAC-SHA1',
                  'SignatureNonce': uuid.uuid4().hex, 'SignatureVersion': '1.0', 'TemplateCode': tc,
                  'TemplateParam': json.dumps({'code': code}, ensure_ascii=False),
                  'Timestamp': datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ'),
                  'Version': '2017-05-25'}
        qs = _up.urlencode(sorted(params.items()))
        string_to_sign = 'GET&%2F&' + _up.quote(qs, safe='~')
        sig = base64.b64encode(hmac.new((sk + '&').encode(), string_to_sign.encode(), hashlib.sha1).digest()).decode()
        with urllib.request.urlopen('https://dysmsapi.aliyuncs.com/?' + qs + '&Signature=' + _up.quote(sig, safe='~'), timeout=15) as r:
            j = json.loads(r.read().decode())
        return (j.get('Code') == 'OK'), j.get('Message') or j.get('Code') or '发送失败'
    except Exception as e:
...
def inquiry_vendor_auth(token):
    ...
    if not phone or not code:
        conn.close(); return jsonify({'error': '请输入手机号和验证码'}), 400
    _reg_ph = (phone == str(s['phone'] or '').strip())
    # 短信验证码校验(5分钟内有效)
    _sms_ok = False
    try:
        if s['sms_code'] and code == str(s['sms_code'] or '').strip() and s['sms_exp']:
            if datetime.datetime.strptime(s['sms_exp'], '%Y-%m-%d %H:%M:%S') >= datetime.datetime.now():
                _sms_ok = True
    except Exception:
        pass
    if _reg_ph and (_sms_ok or code == str(s['access_code'] or '').strip()):
    ...
    conn.close()
    return jsonify({'error': '手机号或验证码不正确（请核对登记手机号；短信验证码5分钟内有效，或使用采购方发送的访问密码）'}), 403
...
def _inq_gate_html(token):
    """V11.236/240 访问门禁: 手机号(本单登记) + 验证码(短信验证码优先; 短信通道未开通时可用采购方分配的访问密码)
...
def inquiry_vendor_page(token):
    ...
    for idx, it in enumerate(items):
        ...
        _rows_html.append((
            ...
            'oninput="calc()" data-q="%s" id="ip%d" value="%s" style="width:76px;padding:5px 6px;border:1px solid #d0d7e2;border-radius:6px;font-size:13px;text-align:right"></td>'
            ...
            '<td style="padding:6px 8px;border-bottom:1px solid #eef;text-align:right;color:#888;white-space:nowrap">¥<span id="ex%d">0.00</span></td>'
    ...
    body = ('<div style="max-width:860px;margin:40px auto;background:#fff;border-radius:12px;padding:28px;'
            ...
            '<p style="color:#888;font-size:13px;margin:0 0 10px">尊敬的 %s，请逐项填写<b>含税单价</b>与<b>税率</b>（默认13%%，可修改，如0%%），不含税单价与总价自动计算；交付日期按<b>天数</b>、质保时间按<b>月数</b>填写</p>%s%s%s'
            ...
            '<script>'
            'window.calc=function(){let t=0;document.querySelectorAll("[id^=ip]").forEach((e,i)=>{const q=parseFloat(e.getAttribute("data-q"))||1;const ip=parseFloat(e.value)||0;'
            'const tr0=(document.getElementById("tx"+i)||{}).value;const tr=(tr0===""||tr0===undefined||isNaN(parseFloat(tr0)))?13:parseFloat(tr0);'
            'const ex=(tr>0)?ip/(1+tr/100):ip;t+=ip*q;'
            'const h=document.getElementById("ex"+i);if(h)h.textContent=ex.toFixed(2);'
            ...
            'window.quickFill=function(){const v=prompt("请输入报价总金额(元,含税):");if(!v||isNaN(v))return;const es=document.querySelectorAll("[id^=ip]");const n=es.length;'
            'const per=parseFloat(v)/n;es.forEach((e,i)=>{e.value=per.toFixed(2)});calc();'
            'alert("已按平均分摊到每行(含税单价)，可再逐行微调")};'
            'window.sub=async function(){const rows=document.querySelectorAll("[id^=ip]");const details=[];let emptyIdx=[];'
            ...
            'const ex=Math.round((tr>0?ip/(1+tr/100):ip)*100)/100;'
            'details.push({unit_price:Math.round(ip*100)/100,excl_price:ex,tax_rate:tr,qty:parseFloat(e.getAttribute("data-q"))||1,'
...
def inquiry_vendor_quote(token):
    ...
    if details:
        try:
            ...
            pass
    # V11.270 用户要求(改为含税单价录入): 明细以含税单价为基准 → 服务端倒算不含税单价(防前端篡改); 兼容旧式仅传不含税单价
    if details:
        for _x in details:
            try:
...
def api_create_repair():
    ...
    V11.221: 保存草稿宽松; 必填部件/附件强校验在 submit(提交报修)时执行;
    自动记录距上次维修完成天数(设备使用周期, 服务商质量评估用)"""
    d = request.json or {}
    ...
    no = _repair_no()
    c = db()
    # V11.221: 同设备(device_no/设备名)上次维修完成时间 → 距今天数=设备使用周期(服务商质量评估数据)
    prev_gap = 0
    dev_no = str(d.get('device_no') or '').strip()
    try:
        _last = c.execute("SELECT actual_finish FROM repair_plans WHERE device_no=? AND status='已归档' AND actual_finish!='' ORDER BY id DESC LIMIT 1", (dev_no,)).fetchone() if dev_no else None
        if not _last:
            _last = c.execute("SELECT actual_finish FROM repair_plans WHERE device_name=? AND status='已归档' AND actual_finish!='' ORDER BY id DESC LIMIT 1", (device,)).fetchone()
        if _last and _last['actual_finish']:
            try:
                from datetime import datetime as _dt
                prev_gap = max(0, (_dt.now() - _dt.strptime(str(_last['actual_finish'])[:19], '%Y-%m-%d %H:%M:%S')).days)
            except Exception:
                try:
                    prev_gap = max(0, (_dt.now() - _dt.strptime(str(_last['actual_finish'])[:10], '%Y-%m-%d')).days)
                except Exception:
                    prev_gap = 0
    except Exception:
        prev_gap = 0
    ...
    for it in (d.get('items') or []):
        if str(it.get('part_name') or '').strip():
            c.execute("INSERT INTO repair_items(plan_id,part_name,fault_note) VALUES(?,?,?)", (pid, it['part_name'].strip(), it.get('fault_note', '')))
    c.commit(); c.close()
    log(session['user_name'], '新建维修报修单', f'{no} {device} 部件{len(d.get("items") or [])}项 附件{len(atts)}个')
...
def api_repair_submit(rid):
    ...
    V11.221: 提交前强校验 故障部件名称≥1项 + 附件两类(损坏照片/手写故障说明)必传"""
    d = request.json or {}
    c = db()
    ...
    # V11.221 2.0表单必填: 故障部件名称至少一项
    it_cnt = c.execute("SELECT COUNT(*) FROM repair_items WHERE plan_id=? AND part_name!=''", (rid,)).fetchone()[0]
    if it_cnt == 0:
        c.close(); return jsonify({'error': '请填写故障部件名称(如线圈/壳体/高压胶管/油封/阀门管件)，至少一项'}), 400
    # V11.221 2.0必传附件: 损坏照片 + 手写故障说明(两类合计≥2且均需存在)
    try:
        atts = json.loads(r['attachments'] or '[]')
    except Exception:
        atts = []
    if len(atts) < 2:
        c.close(); return jsonify({'error': '必传附件：①故障部件损坏照片 ②手写故障说明(写明故障点)，各至少1张，缺一不可提交'}), 400
...
def api_repair_damage(rid):
    ...
    # 定损人权限: V11.221 2.0 采购专员定损三分支 — 采购员/部门负责人可定损(原仅分管领导), 管理员可代
    if session.get('user_role') not in ('系统管理员', '分管领导', '总经理', '采购员', '部门负责人'):
...
def api_repair_accept(rid):
    ...
    V11.221 2.0验收分大小件:
      小件(高压胶管/阀件等) = 申请部门负责人单方线上验收(意见+现场照片);
      大件(大型电机/减速机) = 机电部+申请部门领导+采购专员 三方线上签字(accept_signs JSON);
      验收场景 accept_scene: 服务商出厂验收 / 回厂安装后验收"""
    d = request.json or {}
    c = db()
    r = c.execute("SELECT * FROM repair_plans WHERE id=?", (rid,)).fetchone()
    if not r: c.close(); return jsonify({'error': '维修单不存在'}), 404
    ...
    # V11.221: 验收场景(出厂/回厂) 默认回厂安装后验收
    scene = str(d.get('scene') or '').strip() or '回厂安装后验收'
    if scene not in ('服务商出厂验收', '回厂安装后验收'):
        scene = '回厂安装后验收'
    iclass = str(r['item_class'] or d.get('item_class') or '').strip() or '小件'
    ...
    if iclass == '大件' and result == 'pass':
        ...
        if missing:
            c.close(); return jsonify({'error': '大件验收通过需三方线上签字齐全(缺:' + '、'.join(missing) + ')，每方须填姓名+验收意见'}), 400
    files = json.dumps(d.get('files') or [], ensure_ascii=False)
    signs_json = json.dumps(signs, ensure_ascii=False) if signs else json.dumps([], ensure_ascii=False)
    if result == 'pass':
        c.execute("UPDATE repair_plans SET accept_result='通过', accept_opinion=?, accept_files=?, accept_time=?, outer_status='已回厂', status='验收通过', item_class=?, accept_scene=?, accept_signs=?, updated_at=? WHERE id=?",
                  (opinion, files, now(), iclass, scene, signs_json, now(), rid))
    else:
        c.execute("UPDATE repair_plans SET accept_result='不通过返修', accept_opinion=?, accept_files=?, item_class=?, accept_scene=?, accept_signs=?, status='验收不通过返修', updated_at=? WHERE id=?",
                  (opinion or '退回服务商返修', files, iclass, scene, signs_json, now(), rid))
    c.commit(); c.close()
    _lg = f'{r["plan_no"]} {"性能验收通过" if result=="pass" else "退回服务商返修"}({iclass}/{scene})'
    log(session['user_name'], '验收' + ('通过' if result == 'pass' else '不通过'), _lg)
    return jsonify({'success': True, 'item_class': iclass, 'scene': scene})
...
@app.route('/api/repairs/<int:rid>/offline-paper', methods=['POST'])
@login_required
def api_repair_offline_paper(rid):
    """V11.221 2.0缺口①: 加急线下审批兜底 — 上传线下签字纸质件归档 + (可选)线下审批直接通过
    body: {files:[upload路径...], opinion:线下签字说明, approve:true=视同三级审批已线下完成(仅加急单且状态=定损完成待审批)}
    线下签字件永久归档, 全程留痕可追溯; 非加急单仍强制线上审批流"""
    d = request.json or {}
    c = db()
    r = c.execute("SELECT * FROM repair_plans WHERE id=?", (rid,)).fetchone()
    if not r: c.close(); return jsonify({'error': '维修单不存在'}), 404
    ...
    opinion = str(d.get('opinion') or '').strip()
    if not files: c.close(); return jsonify({'error': '请上传线下签字纸质件(照片/扫描件)'}), 400
    if r['status'] not in ('草稿', '待定损', '定损完成待审批', '审批驳回'):
        c.close(); return jsonify({'error': f'当前状态({r["status"]})不可归档线下签字件'}), 400
    # 追加归档(JSON数组, 永久留痕)
    try:
        op = json.loads(r['offline_papers'] or '[]')
    except Exception:
        op = []
    op.append({'files': files, 'by': op_name, 'opinion': opinion or '线下签字纸质件归档', 'time': now()})
    c.execute("UPDATE repair_plans SET offline_papers=? WHERE id=?", (json.dumps(op, ensure_ascii=False), rid))
    # 加急线下审批兜底: 加急单且在审批节点 → 视同三级审批线下完成, 直接流转审批通过(仅限加急)
    approve = bool(d.get('approve'))
    is_urgent = (r['urgency'] == '紧急')
    flowed = False
    if approve:
        if not is_urgent:
            c.commit(); c.close(); return jsonify({'error': '仅【紧急】单据支持线下审批兜底，普通单据必须走线上审批流'}), 400
        if r['status'] != '定损完成待审批':
            c.commit(); c.close(); return jsonify({'error': f'当前状态({r["status"]})不在审批节点，无法线下审批通过'}), 400
        c.commit(); c.close()
        # 复用统一审批办结: 全部审批实例approved + 父单据状态流转审批通过(带'线下签字'留痕)
        ok = finish_approvals('repair_plan', rid, 'ok', op_name, session.get('user_id', 0),
                              comment=f'加急线下审批通过({op_name}) 纸质签字件已归档', attachments=files)
        # finish_approvals 仅在无pending时改父状态; 加急线下=全部节点一次通过
        cc = db()
        pend = cc.execute("SELECT COUNT(*) FROM approval_instances WHERE biz_type='repair_plan' AND biz_id=? AND status='pending'", (rid,)).fetchone()[0]
        if pend > 0:
            cc.execute("UPDATE approval_instances SET status='approved', approver=?, approver_id=?, comment='加急线下签字通过', processed_at=? WHERE biz_type='repair_plan' AND biz_id=? AND status='pending'",
                       (op_name, session.get('user_id', 0), now(), rid))
            cc.execute("UPDATE repair_plans SET status='审批通过', updated_at=? WHERE id=?", (now(), rid))
            cc.commit()
        cc.close()
        flowed = True
    else:
...
def api_dashboard_meta():
    ...
    c = db()
    yrs = [r[0] for r in c.execute("SELECT DISTINCT substr(created_at,1,4) y FROM purchase_orders WHERE COALESCE(created_at,'')!='' ORDER BY y DESC")]
    for t, col in (('purchase_requests', 'created_at'), ('repair_plans', 'created_at'), ('contracts', 'sign_date')):
        try:
            yrs += [r[0] for r in c.execute("SELECT DISTINCT substr(%s,1,4) y FROM %s WHERE COALESCE(%s,'')!=''" % (col, t, col))]
        except Exception:
            pass
    yrs = sorted({y for y in yrs if y and y.isdigit()}, reverse=True) or [str(datetime.date.today().year)]
    sups = [r[0] for r in c.execute("SELECT DISTINCT supplier FROM purchase_orders WHERE COALESCE(supplier,'')!='' ORDER BY supplier")]
    cats = set(r[0] for r in c.execute("SELECT DISTINCT category FROM purchase_orders WHERE COALESCE(category,'')!=''"))
    try:
        cats |= set(r[0] for r in c.execute("SELECT name FROM categories WHERE COALESCE(name,'')!=''"))
    except Exception:
        pass
    deps = set(r[0] for r in c.execute("SELECT DISTINCT dept FROM purchase_requests WHERE COALESCE(dept,'')!=''"))
    for t in ('receivings', 'requisitions', 'repair_plans'):
        try:
            deps |= set(r[0] for r in c.execute("SELECT DISTINCT dept FROM %s WHERE COALESCE(dept,'')!=''" % t))
        except Exception:
            pass
    whs = set(r[0] for r in c.execute("SELECT DISTINCT warehouse FROM inventory WHERE COALESCE(warehouse,'')!=''"))
    try:
        whs |= set(r[0] for r in c.execute("SELECT DISTINCT warehouse FROM receivings WHERE COALESCE(warehouse,'')!=''"))
    except Exception:
        pass
    c.close()
    return jsonify({'years': yrs, 'suppliers': sups, 'categories': sorted(cats), 'departments': sorted(deps),
                    'warehouses': sorted(whs), 'scope': _dash_scope(),
                    'role': session.get('user_role', '')})


# ============================================================
# V11.275 数据看板 · 统一筛选引擎
#   用户反馈: 顶部筛选"全部都是死的, 切换后下面的数据根本没有变动"。
#   实测根因: 各 Tab 只认自己那点参数(财务/维修只认 year, 库存只认 warehouse),
#             且前端"时间范围"从未随请求发出 → 全部形同装饰。
#   本引擎: ①按每张表的真实列(PRAGMA)自动挂筛选, 缺列自动跳过(防 no such column)
#           ②时间范围覆盖会计年度(近12个月/本季/本月)
#           ③5 个 Tab(卡片/图表/明细)走同一套口径, 保证"总数=明细合计"
# ============================================================
_DASH_COLC = {}
_DASH_STATUS_EX = {'purchase_requests': " AND status NOT IN ('草稿','已驳回','已作废','已撤销')",
                   ...
                   'invoices': " AND status NOT IN ('作废')",
                   'repair_plans': " AND status NOT IN ('草稿','已驳回','已作废')"}


def _dcol(tbl):
    if tbl not in _DASH_COLC:
        try:
            _c = db()
            _DASH_COLC[tbl] = {r[1] for r in _c.execute('PRAGMA table_info(%s)' % tbl).fetchall()}
            _c.close()
        except Exception:
            _DASH_COLC[tbl] = set()
    return _DASH_COLC[tbl]


def _dash_f():
    return {'year': (request.args.get('year') or '').strip(),
            'range': (request.args.get('range') or '').strip().lower(),
...
def _dash_axis(f):
    """时段轴: 返回 (月份键列表 YYYY-MM, 轴标签, 起始日 or None)"""
    ...
    if rng == '12m':
        ...
        for i in range(11, -1, -1):
            ...
            while m2 <= 0:
                m2 += 12; y2 -= 1
            keys.append('%04d-%02d' % (y2, m2))
        labels = [(('%s年%d月' % (k[:4], int(k[5:7]))) if k[:4] != str(yy) else ('%d月' % int(k[5:7]))) for k in keys]
        return keys, labels, keys[0] + '-01'
    if rng == 'q':
        q0 = ((today.month - 1) // 3) * 3 + 1
        keys = ['%04d-%02d' % (today.year, q0 + i) for i in range(3)]
        return keys, ['%d月' % (q0 + i) for i in range(3)], keys[0] + '-01'
    if rng == 'm':
        keys = ['%04d-%02d' % (today.year, today.month)]
        return keys, ['%d月' % today.month], keys[0] + '-01'
    yr = f['year'] or str(today.year)
    if not str(yr).isdigit():
        yr = str(today.year)
    keys = ['%04d-%02d' % (int(yr), i) for i in range(1, 13)]
    return keys, ['%d月' % i for i in range(1, 13)], None


def _dash_w(f, tbl, alias='', date_col='created_at', sup_col='supplier',
            use_dept=True, use_cat=True, use_wh=True, with_status=True):
    """统一筛选 SQL 片段 + 参数(按表真实列; 该表没有的筛选维度自动跳过)"""
    ...
    if with_status:
        sql += _DASH_STATUS_EX.get(tbl, '')
    if f['supplier'] and sup_col and sup_col in cols:
        sql += ' AND %s%s=?' % (a, sup_col); ps.append(f['supplier'])
    if f['category'] and use_cat:
        if 'category' in cols:
...
def _dash_mfill(c, f, sql, ps=(), div=10000.0):
    """按月(YYYY-MM)聚合 → 按当前时段轴对齐; div=0 表示不换算"""
    keys, labels, _s = _dash_axis(f)
    d = {}
    for r in c.execute(sql, ps).fetchall():
        d[r[0]] = r[1] or 0
    out = []
    for k in keys:
        v = d.get(k, 0)
...
def _dash_scope_ps():
    """own 域过滤(仅普通员工): 各表人员列不同, 分表使用"""
    if _dash_scope() != 'own':
        return None, []
    nm, uid = session.get('user_name', ''), session.get('user_id', 0)
    return {'req': " AND (requester=? OR requester_id=?)", 'ord': " AND (requester=? OR owner_id=?)",
            'inq': ' AND created_by=?', 'inqa': ' AND i.created_by=?'}, [nm, uid]


@app.route('/api/dashboard/overview')
@login_required
def api_dashboard_overview():
    """Tab1 经营总览: 6指标卡 + 近5年柱线对比 + 时段月度趋势 + 4组明细(全部受统一筛选)"""
    f = _dash_f()
    y = int(f['year'] or datetime.date.today().year)
    c = db()
    sc, own_ps = _dash_scope_ps()
    own = sc is not None
    ...
    # --- 核心指标(与明细同口径) ---
    this_o = q1("SELECT COALESCE(SUM(total_amount),0), COUNT(*) FROM purchase_orders WHERE 1=1" + wo + (sc['ord'] if own else ''), tuple(po_o) + (tuple(own_ps) if own else ()))
    _f_last = dict(f); _f_last['year'] = pre; _f_last['range'] = ''
    _k, _l, _st = _dash_axis(_f_last)
    wo_pre, po_o_pre = _dash_w(_f_last, 'purchase_orders')
    last_o = q1("SELECT COALESCE(SUM(total_amount),0), COUNT(*) FROM purchase_orders WHERE 1=1" + wo_pre + (sc['ord'] if own else ''), tuple(po_o_pre) + (tuple(own_ps) if own else ()))
    ...
    _ys = str(y) + '-01-01'
    _supq = ' AND supplier=?' if f['supplier'] else ''
    _supv = tuple([f['supplier']] if f['supplier'] else [])
    con_pre = ex1("SELECT COALESCE(SUM(amount),0) FROM contracts WHERE status NOT IN ('已驳回','已作废') AND COALESCE(sign_date,'')!='' AND sign_date<?" + _supq, (_ys,) + _supv)
    pay_pre = ex1("SELECT COALESCE(SUM(amount),0) FROM payment_requests WHERE status NOT IN ('草稿','已驳回','已作废') AND COALESCE(COALESCE(NULLIF(paid_at,''),created_at),'')!='' AND COALESCE(NULLIF(paid_at,''),created_at)<?" + _supq, (_ys,) + _supv)
    bal_pre = con_pre - pay_pre
    # 库存(现量, 不受时间影响; 受仓库/类别筛选)
    inv_val = ex1("SELECT COALESCE(SUM(quantity*price),0) FROM inventory WHERE 1=1" + wi, tuple(po_i))
    inv_qty = ex1("SELECT COALESCE(SUM(quantity),0) FROM inventory WHERE 1=1" + wi, tuple(po_i))
    rep_amt = ex1("SELECT COALESCE(SUM(CASE WHEN invoice_amount>0 THEN invoice_amount WHEN quote_total>0 THEN quote_total ELSE est_cost END),0) FROM repair_plans WHERE 1=1" + wrep + (sc['req'] if own else ''), tuple(po_rep) + (tuple(own_ps) if own else ()))
    rep_amt_last = ex1("SELECT COALESCE(SUM(CASE WHEN invoice_amount>0 THEN invoice_amount WHEN quote_total>0 THEN quote_total ELSE est_cost END),0) FROM repair_plans WHERE 1=1" + _dash_w(_f_last, 'repair_plans', sup_col='repair_company')[0] + (sc['req'] if own else ''),
                       tuple(_dash_w(_f_last, 'repair_plans', sup_col='repair_company')[1]) + (tuple(own_ps) if own else ()))
    ...
    cards = [
        ...
        {'k': '平均单笔金额', 'v': round(avg_price, 2), 'unit': '元/笔', 'cmp': yoy(avg_last - avg_price, avg_last) if avg_last else None, 'cmp_label': '降本'},
        ...
        {'k': '维修费用总额', 'v': round(rep_amt / 10000, 2), 'unit': '万元', 'cmp': yoy(rep_amt, rep_amt_last), 'cmp_label': '同比'},
    ]
    # --- 近5年采购金额 + 降本(受非时间类筛选) ---
    years5 = []
    for yy in range(y - 4, y + 1):
        _fy = dict(f); _fy['year'] = str(yy); _fy['range'] = ''
        w_y, p_y = _dash_w(_fy, 'purchase_orders')
        w_ry, p_ry = _dash_w(_fy, 'purchase_requests', sup_col=None)
        o = ex1("SELECT COALESCE(SUM(total_amount),0) FROM purchase_orders WHERE 1=1" + w_y + (sc['ord'] if own else ''), tuple(p_y) + (tuple(own_ps) if own else ()))
        rq = ex1("SELECT COALESCE(SUM(total_estimated),0) FROM purchase_requests WHERE 1=1" + w_ry + (sc['req'] if own else ''), tuple(p_ry) + (tuple(own_ps) if own else ()))
        years5.append({'year': yy, 'amt': round(o / 10000, 2), 'save': round(max(rq - o, 0) / 10000, 2)})
    # --- 时段月度采购趋势 ---
    months, labels_m = _dash_mfill(c, f, "SELECT substr(created_at,1,7) k, COALESCE(SUM(total_amount),0) FROM purchase_orders WHERE 1=1" + wo + (sc['ord'] if own else '') + " GROUP BY k",
                                   tuple(po_o) + (tuple(own_ps) if own else ()))
    # --- 明细(与卡片同口径) ---
    lim = ' LIMIT 200'
    material_drop = _dash_rows(c, "SELECT pr.req_no, ri.item_name, ri.spec, pr.dept, pr.total_estimated est FROM purchase_requests pr JOIN request_items ri ON ri.req_id=pr.id WHERE 1=1" +
                               wr.replace(' AND substr(created_at,1,4)=?', ' AND substr(pr.created_at,1,4)=?').replace(' AND created_at>=?', ' AND pr.created_at>=?').replace(' AND dept=?', ' AND pr.dept=?').replace(' AND status NOT IN', ' AND pr.status NOT IN') + (sc['req'] if own else '') + ' ORDER BY pr.created_at DESC' + lim,
                               tuple(po_r) + (tuple(own_ps) if own else ()))
    supplier_rank = _dash_rows(c, "SELECT supplier, COALESCE(SUM(total_amount),0) amt, COUNT(*) n FROM purchase_orders WHERE 1=1" + wo + (sc['ord'] if own else '') + " GROUP BY supplier ORDER BY amt DESC LIMIT 10",
                               tuple(po_o) + (tuple(own_ps) if own else ()))
    ap_detail = _dash_rows(c, "SELECT c.contract_no, c.supplier, c.amount, COALESCE((SELECT SUM(p.amount) FROM payment_requests p WHERE p.contract_id=c.id AND p.status NOT IN ('草稿','已驳回','已作废')),0) paid FROM contracts c WHERE 1=1" + wc_nt + " ORDER BY c.amount DESC" + lim, tuple(po_c_nt))
    _fy_prev = dict(f); _fy_prev['year'] = pre; _fy_prev['range'] = ''
    w_prev, p_prev = _dash_w(_fy_prev, 'purchase_orders')
    sup_y = _dash_rows(c, "SELECT supplier, COALESCE(SUM(total_amount),0) amt, COUNT(*) n FROM purchase_orders WHERE 1=1" + wo + (sc['ord'] if own else '') + " GROUP BY supplier", tuple(po_o) + (tuple(own_ps) if own else ()))
    sup_p = {r['supplier']: r['amt'] for r in _dash_rows(c, "SELECT supplier, COALESCE(SUM(total_amount),0) amt FROM purchase_orders WHERE 1=1" + w_prev + (sc['ord'] if own else '') + " GROUP BY supplier", tuple(p_prev) + (tuple(own_ps) if own else ()))}
    supplier_save = []
    for r in sup_y:
        last = sup_p.get(r['supplier'], 0)
...
def api_dashboard_purchase():
    """Tab2 采购分析: 6指标 + 申请vs订单双柱 + 类别占比环 + 供应商TOP10横柱 + 3明细(全部受统一筛选)"""
    f = _dash_f()
    c = db()
    sc, own_ps = _dash_scope_ps()
    own = sc is not None
    ...
    def ex1(sql, ps=()):
        r = c.execute(sql, ps).fetchone()
        return r[0] if r else 0
    rq_n = c.execute("SELECT COUNT(*), COALESCE(SUM(total_estimated),0) FROM purchase_requests WHERE 1=1" + wr + (sc['req'] if own else ''), tuple(po_r) + (tuple(own_ps) if own else ())).fetchone()
    od_n = c.execute("SELECT COUNT(*), COALESCE(SUM(total_amount),0) FROM purchase_orders WHERE 1=1" + wo + (sc['ord'] if own else ''), tuple(po_o) + (tuple(own_ps) if own else ())).fetchone()
    inq_n = ex1("SELECT COUNT(*) FROM inquiries WHERE 1=1" + wi + (sc['inq'] if own else ''), tuple(po_i) + ((session.get('user_name', ''),) if own else ()))
    save = max(rq_n[1] - od_n[1], 0)
    cards = [
        {'k': '采购申请总数', 'v': rq_n[0], 'unit': '笔', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '采购申请总金额', 'v': round(rq_n[1] / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '采购订单总数', 'v': od_n[0], 'unit': '笔', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '采购订单总金额', 'v': round(od_n[1] / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '询价比价次数', 'v': inq_n, 'unit': '次', 'cmp': None, 'cmp_label': f['range'] or '本年'},
    ...
    lim = ' LIMIT 200'
    rq_det = _dash_rows(c, "SELECT req_no, dept, requester, created_at, purpose, total_estimated, status FROM purchase_requests WHERE 1=1" + wr + (sc['req'] if own else '') + " ORDER BY created_at DESC" + lim, tuple(po_r) + (tuple(own_ps) if own else ()))
    od_det = _dash_rows(c, "SELECT order_no, supplier, total_amount, created_at, status, trade_mode FROM purchase_orders WHERE 1=1" + wo + (sc['ord'] if own else '') + " ORDER BY created_at DESC" + lim, tuple(po_o) + (tuple(own_ps) if own else ()))
...
def api_dashboard_inventory():
    """Tab3 库存分析: 6指标 + 入出库双折线 + 类别占比环 + 季度周转柱 + 4明细(全部受统一筛选)"""
    f = _dash_f()
    c = db()
    sc, own_ps = _dash_scope_ps()
    own = sc is not None
    wi, po_i = _dash_w(f, 'inventory', date_col=None, sup_col=None, use_dept=False)
    wrv, po_rv = _dash_w(f, 'receivings', use_cat=False)
    wq, po_q = _dash_w(f, 'requisitions', sup_col=None, use_cat=False, use_wh=False)
    ...
    inv_qty = ex1("SELECT COALESCE(SUM(quantity),0) FROM inventory WHERE 1=1" + wi, tuple(po_i))
    inv_val = ex1("SELECT COALESCE(SUM(quantity*price),0) FROM inventory WHERE 1=1" + wi, tuple(po_i))
    low_n = ex1("SELECT COUNT(*) FROM inventory WHERE quantity < safe_stock AND safe_stock > 0" + wi, tuple(po_i))
    in_q = ex1("SELECT COALESCE(SUM(quantity),0) FROM receivings WHERE 1=1" + wrv, tuple(po_rv))
    out_q = ex1("SELECT COALESCE(SUM(quantity),0) FROM requisitions WHERE 1=1" + wq + (sc['req'] if own else ''), tuple(po_q) + (tuple(own_ps) if own else ()))
    in_amt = ex1("SELECT COALESCE(SUM(CASE WHEN est_amount>0 THEN est_amount ELSE quantity*(SELECT COALESCE(price,0) FROM inventory i WHERE i.item_name=r.item_name LIMIT 1) END),0) FROM receivings r WHERE 1=1" +
                 wrv.replace(' AND warehouse=?', ' AND r.warehouse=?').replace(' AND dept=?', ' AND r.dept=?').replace(' AND supplier=?', ' AND r.supplier=?'), tuple(po_rv))
    turn = round((out_q * (inv_val / inv_qty if inv_qty else 0)) / (inv_val or 1), 2) if inv_qty else 0
    cards = [
        {'k': '当前库存总数量', 'v': inv_qty, 'unit': '', 'cmp': None, 'cmp_label': '现量'},
        {'k': '当前库存总金额', 'v': round(inv_val / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': '现值'},
        {'k': '入库总数量', 'v': in_q, 'unit': '', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '出库总数量', 'v': out_q, 'unit': '', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '库存周转率', 'v': turn, 'unit': '次', 'cmp': None, 'cmp_label': '估算'},
        {'k': '库存预警物料数', 'v': low_n, 'unit': '种', 'cmp': None, 'cmp_label': '低于安全库存'},
    ]
    in_m, labels = _dash_mfill(c, f, "SELECT substr(created_at,1,7) k, COALESCE(SUM(quantity),0) FROM receivings WHERE 1=1" + wrv + " GROUP BY k", tuple(po_rv), div=0)
    out_m, _l = _dash_mfill(c, f, "SELECT substr(created_at,1,7) k, COALESCE(SUM(quantity),0) FROM requisitions WHERE 1=1" + wq + (sc['req'] if own else '') + " GROUP BY k", tuple(po_q) + (tuple(own_ps) if own else ()), div=0)
    # 类别占比(库存金额)
    raw = c.execute("SELECT COALESCE(cat_code,'其他') t, COALESCE(SUM(quantity*price),0) a FROM inventory WHERE 1=1" + wi + " GROUP BY t", tuple(po_i)).fetchall()
    cats = {}
    for code, a in raw:
        nm = '未分类'
        try:
            ...
            pass
        cats[nm] = cats.get(nm, 0) + (a or 0)
    if f['category']:
        cats = {k: v for k, v in cats.items() if k == f['category']} or {f['category']: 0}
    cat_list = [{'name': k, 'value': round(v, 0)} for k, v in cats.items()] or [{'name': '暂无', 'value': 0}]
    # 季度周转(选定年度的 Q1..Q4; 其余筛选同样生效)
    yr_sel = str(f['year'] or datetime.date.today().year)
    q_labels, q_turn = ['Q1', 'Q2', 'Q3', 'Q4'], []
    qdat = wq if ('substr(created_at,1,4)=?' in wq) else None
    rdat = wrv if ('substr(created_at,1,4)=?' in wrv) else None
    for qq in range(1, 5):
        m0, m1 = (qq - 1) * 3 + 1, qq * 3
        qout = qin = 0
        if qdat:
            wqk = qdat.replace(' AND substr(created_at,1,4)=?', ' AND CAST(substr(created_at,6,2) AS INTEGER) BETWEEN ? AND ?')
        else:
            wqk = wq + ' AND CAST(substr(created_at,6,2) AS INTEGER) BETWEEN ? AND ?'
        qout = ex1("SELECT COALESCE(SUM(quantity),0) FROM requisitions WHERE 1=1" + wqk + (sc['req'] if own else ''),
                   ((m0, m1) + (tuple(po_q[1:]) if qdat else tuple(po_q))) + (tuple(own_ps) if own else ()))
        q_turn.append(round((qout or 0) / ((inv_val or 0) + 1) * 100, 2))
    lim = ' LIMIT 200'
    inv_det = _dash_rows(c, "SELECT item_name, spec, unit, quantity, price, (quantity*price) amt, warehouse FROM inventory WHERE 1=1" + wi + " ORDER BY amt DESC" + lim, tuple(po_i))
...
def api_dashboard_finance():
    ...
    cards = [
        {'k': '合同总金额', 'v': round(con_amt / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': f['range'] or '本年签订'},
        {'k': '已开票金额', 'v': round(inv_amt / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '未开票金额', 'v': round(max(con_amt - inv_amt, 0) / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': '差额'},
        {'k': '已付款金额', 'v': round(pay_amt / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '应付账款余额', 'v': round(ap_bal / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': '在账'},
        {'k': '发票催收预警数', 'v': due_n, 'unit': '个', 'cmp': None, 'cmp_label': '合同'},
    ]
    con_m, labels = _dash_mfill(c, f, "SELECT substr(sign_date,1,7) k, COALESCE(SUM(amount),0) FROM contracts WHERE 1=1" + wc + " GROUP BY k", tuple(po_c))
    inv_m, _l = _dash_mfill(c, f, "SELECT substr(created_at,1,7) k, COALESCE(SUM(amount),0) FROM invoices WHERE 1=1" + wv + " GROUP BY k", tuple(po_v))
    pay_m, _l2 = _dash_mfill(c, f, "SELECT substr(COALESCE(NULLIF(paid_at,''),created_at),1,7) k, COALESCE(SUM(amount),0) FROM payment_requests WHERE 1=1" + wp + " GROUP BY k", tuple(po_p))
    # 账龄(受筛选; 未收票/未付款的合同按签订时间分桶)
    aging = {'30天内': 0, '30-60天': 0, '60-90天': 0, '90天以上': 0}
    today = datetime.date.today()
    _wage = wc_nt
    for r in c.execute("SELECT c.sign_date, c.amount, COALESCE((SELECT SUM(p.amount) FROM payment_requests p WHERE p.contract_id=c.id AND p.status NOT IN ('草稿','已驳回','已作废')),0) paid FROM contracts c WHERE 1=1" + _wage, tuple(po_c_nt)):
        amt = (r[1] or 0) - (r[2] or 0)
        if amt <= 0:
            continue
        try:
            sd = datetime.datetime.strptime((r[0] or '')[:10], '%Y-%m-%d').date()
            days = (today - sd).days
        except Exception:
            days = 999
        if days <= 30:
    ...
    lim = ' LIMIT 200'
    con_det = _dash_rows(c, "SELECT contract_no, supplier, amount, sign_date, status, inv_collect_status FROM contracts WHERE 1=1" + wc + " ORDER BY amount DESC" + lim, tuple(po_c))
...
def api_dashboard_repair():
    """Tab5 维修分析: 6指标 + 费用面积趋势 + 类型占比环 + 设备TOP10 + 3明细(全部受统一筛选)"""
    f = _dash_f()
    c = db()
    sc, own_ps = _dash_scope_ps()
    own = sc is not None
    wp, po_p = _dash_w(f, 'repair_plans', sup_col='repair_company')
    cost = "CASE WHEN invoice_amount>0 THEN invoice_amount WHEN quote_total>0 THEN quote_total ELSE est_cost END"
    sfx = wp + (sc['req'] if own else '')
    def _rpq(frag):
        """JOIN repair_plans 后, 统一筛选片段里的列必须加 rp. 前缀(否则 ambiguous column name)"""
        return (frag.replace(' AND substr(created_at,1,4)=?', ' AND substr(rp.created_at,1,4)=?')
                    .replace(' AND created_at>=?', ' AND rp.created_at>=?')
                    .replace(' AND dept=?', ' AND rp.dept=?')
                    .replace(' AND repair_company=?', ' AND rp.repair_company=?')
                    .replace(' AND status NOT IN', ' AND rp.status NOT IN'))
    ps = tuple(po_p) + (tuple(own_ps) if own else ())

    def ex1(sql, p=()):
        r = c.execute(sql, p).fetchone()
        return r[0] if r else 0
    n_all = ex1("SELECT COUNT(*) FROM repair_plans WHERE 1=1" + sfx, ps)
    cost_all = ex1("SELECT COALESCE(SUM(%s),0) FROM repair_plans WHERE 1=1" % cost + sfx, ps)
    cost_ext = ex1("SELECT COALESCE(SUM(%s),0) FROM repair_plans WHERE repair_type='委外维修'" % cost + sfx, ps)
    cost_int = ex1("SELECT COALESCE(SUM(est_cost),0) FROM repair_plans WHERE repair_type='内部自修'" + sfx, ps)
    chg_n = ex1("SELECT COUNT(*) FROM repair_changes rc JOIN repair_plans rp ON rp.id=rc.plan_id WHERE 1=1" + _rpq(wp), ps)
    cyc = ex1("SELECT COALESCE(AVG(julianday(actual_finish)-julianday(created_at)),0) FROM repair_plans WHERE COALESCE(actual_finish,'')!=''" + sfx, ps)
    cards = [
        {'k': '维修工单总数', 'v': n_all, 'unit': '单', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '维修费用总额', 'v': round(cost_all / 10000, 2), 'unit': '万元', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        ...
        {'k': '维修变更次数', 'v': chg_n, 'unit': '次', 'cmp': None, 'cmp_label': f['range'] or '本年'},
        {'k': '平均维修周期', 'v': round(cyc, 1), 'unit': '天', 'cmp': None, 'cmp_label': '报修→完工'},
    ]
    cost_m, labels = _dash_mfill(c, f, "SELECT substr(created_at,1,7) k, COALESCE(SUM(%s),0) FROM repair_plans WHERE 1=1" % cost + sfx + " GROUP BY k", ps)
    t_rows = c.execute("SELECT COALESCE(repair_type,'未定损') t, COUNT(*) n, COALESCE(SUM(%s),0) a FROM repair_plans WHERE 1=1" % cost + sfx + " GROUP BY t", ps).fetchall()
    type_pie = [{'name': r[0], 'n': r[1], 'value': round(r[2], 0)} for r in t_rows] or [{'name': '暂无', 'value': 0}]
    dev_top = c.execute("SELECT device_name, COALESCE(SUM(%s),0) a, COUNT(*) n FROM repair_plans WHERE COALESCE(device_name,'')!=''" % cost + sfx + " GROUP BY device_name ORDER BY a DESC LIMIT 10", ps).fetchall()
    lim = ' LIMIT 200'
